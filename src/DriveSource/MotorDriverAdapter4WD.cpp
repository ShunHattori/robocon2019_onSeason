...
double MotorDriverAdapter4WD::absdouble(double ver)
{
  if (ver < 0)
  {
    return -ver;
...
void MotorDriverAdapter4WD::apply(double pwm[4])
{
  for (int i = 0; i < 4; i++)
  {
  ...
  if (0 < duty[0])
  {
    FRR->write((absdouble(duty[0])));
    FRN->write(0);
  }
  else if (duty[0] < 0)
  {
    FRR->write(0);
    FRN->write(absdouble(duty[0]));
  }
  else
  {
    FRR->write(0);
  ...
  if (0 < duty[1])
  {
    FLR->write(absdouble(duty[1]));
    FLN->write(0);
  }
  else if (duty[1] < 0)
  {
    FLR->write(0);
    FLN->write(absdouble(duty[1]));
  }
  else
  {
    FLR->write(0);
  ...
  if (0 < duty[2])
  {
    BRR->write(absdouble(duty[2]));
    BRN->write(0);
  }
  else if (duty[2] < 0)
  {
    BRR->write(0);
    BRN->write(absdouble(duty[2]));
  }
  else
  {
    BRR->write(0);
  ...
  if (0 < duty[3])
  {
    BLR->write(absdouble(duty[3]));
    BLN->write(0);
  }
  else if (duty[3] < 0)
  {
    BLR->write(0);
    BLN->write(absdouble(duty[3]));
  }
  else
  {
    BLR->write(0);
