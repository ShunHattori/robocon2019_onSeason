...
double MotorDriverAdapter3WD::absdouble(double ver)
{
  if (ver < 0)
  {
    return -ver;
...
void MotorDriverAdapter3WD::apply(double pwm[3])
{
  for (int i = 0; i < 3; i++)
  {
  ...
  if (0 < duty[0])
  {
    FCR->write((absdouble(duty[0])));
    FCN->write(0);
  }
  else if (duty[0] < 0)
  {
    FCR->write(0);
    FCN->write(absdouble(duty[0]));
  }
  else
  {
    FCR->write(0);
  ...
  if (0 < duty[1])
  {
    BRR->write(absdouble(duty[1]));
    BRN->write(0);
  }
  else if (duty[1] < 0)
  {
    BRR->write(0);
    BRN->write(absdouble(duty[1]));
  }
  else
  {
    BRR->write(0);
  ...
  if (0 < duty[2])
  {
    BLR->write(absdouble(duty[2]));
    BLN->write(0);
  }
  else if (duty[2] < 0)
  {
    BLR->write(0);
    BLN->write(absdouble(duty[2]));
  }
  else
  {
    BLR->write(0);
